...
% BOLDfiles=dir('I:\ADNI\MCI\BOLD_DTI\*.mat');
% for i=1:length(BOLDfiles)
%     load([BOLDfiles(i).folder,'\',BOLDfiles(i).name])
%     X{1,i}=theROITimeCoursesTotal';
% end
n_Subjects=size(X,2);
%%%%%%%%%%%%%%
kk=1;
...
for nsub=1:n_Subjects
    ...
    parfor seed=1:N
        ...
        x(find(x<-3*std(x)))=-3*std(x);
        Phase_BOLD_data(seed,:) = angle(hilbert(x));
    end
   T=10:Tmax-10; 
    for t=T
...
for nsub=1:n_Subjects
    ...
    [aux minfreq]=min(abs(freq-0.01));
    [aux maxfreq]=min(abs(freq-0.08));
    nfreqs=length(freq);
...
for we=WE
    ...
    for iter=1:200
        ...
        xs=zeros(Tmax,N);
        %number of iterations, 100 willk锟絟rlich, weil reicht in diesem Fall
        ...
        for t=0:dt:((Tmax-1)*TR)
            num=num+1;
            waitbar(num/(length(0:dt:((Tmax-1)*TR))), f, sprintf('Progress: %d %%', floor(num/(length(0:dt:((Tmax-1)*TR)))*100)));
            suma = wC*z - sumC.*z; % sum(Cij*xi) - sum(Cij)*xj
            zz = z(:,end:-1:1); % flipped z, because (x.*x + y.*y)
            z = z + dt*(a.*z + zz.*omega - z.*(z.*z+zz.*zz) + suma) + dsig*randn(N,2);
